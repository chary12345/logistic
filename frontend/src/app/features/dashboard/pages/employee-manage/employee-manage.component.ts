    ...
    <div class="page-card">
      ...
      <form [formGroup]="form" (ngSubmit)="submit()" class="employee-form">
        ...
        <div class="submit-row">
          ...
          <button mat-stroked-button type="button" (click)="form.reset()"><mat-icon>clear</mat-icon> Reset</button>
        </div>
      </form>
    </div>
  `,
...
export class EmployeeManageComponent implements OnDestroy {
  ...
  submit(): void {
    ...
    this.empSvc.create({ ...v, companyCode: this.auth.companyCode } as any)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: () => { this.loading = false; this.snack.success('Employee created!'); this.form.reset(); },
