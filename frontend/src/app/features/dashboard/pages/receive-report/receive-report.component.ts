...
export class ReceiveReportComponent implements OnDestroy {
  ...
  exportExcel(): void {
    this.exportSvc.exportExcel(this.rowData.map(b => ({
      'LR No': b.loadingReciept, 'Received Date': formatAppDate(b.recieveDate), 'Consignor': b.consignorName,
  ...
  downloadPDF(action: 'download' | 'print'): void {
    ...
    const rows = this.rowData.map(b => [b.loadingReciept, formatAppDate(b.recieveDate), b.consignorName, b.consigneeName,
