...
export class DispatchReportComponent implements OnDestroy {
  ...
  exportExcel(): void {
    this.exportSvc.exportExcel(this.rowData.map(b => ({
      'LR No': b.loadingReciept, 'Dispatch Date': formatAppDate(b.dispatchDate), 'Consignor': b.consignorName,
  ...
  downloadPDF(action: 'download' | 'print'): void {
    ...
    const rows = this.rowData.map(b => [b.loadingReciept, formatAppDate(b.dispatchDate), b.consignorName, b.consigneeName, b.destinationBranchCode, b.billType, calcBookingGrandTotal(b)]);
